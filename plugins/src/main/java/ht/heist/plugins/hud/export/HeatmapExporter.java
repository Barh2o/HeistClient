package ht.heist.plugins.hud.export;
...
public final class HeatmapExporter {
    /**
     * Render to outDir/heatmap-YYYYMMDD-HHmmss.png. Returns absolute path or null
     * on failure.
     */
    public String exportTo(File outDir, List<TapEvent> events, HeistHUDConfig.Palette palette,
        ...
        if (events == null || events.isEmpty() || width <= 0 || height <= 0)
            return null;
        try {
            if (outDir != null && !outDir.exists())
                Files.createDirectories(outDir.toPath());
            File outFile = new File(outDir,
                    "heatmap-" + new SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date()) + ".png");
        ...
        for (TapEvent e : events) {
            ...
            for (int y = minY; y <= maxY; y++) {
                ...
                for (int x = minX; x <= maxX; x++) {
                    int dx = x - px;
                    int d2 = dx * dx + dy * dy;
                    if (d2 > kernelRadius * kernelRadius)
                        continue;
    ...
    private static int colorFor(double t, HeistHUDConfig.Palette palette) {
        ...
        int red = (int) Math.round(c0.getRed() + f * (c1.getRed() - c0.getRed()));
        int grn = (int) Math.round(c0.getGreen() + f * (c1.getGreen() - c0.getGreen()));
        int blu = (int) Math.round(c0.getBlue() + f * (c1.getBlue() - c0.getBlue()));
        int a = (int) Math.round(64 + 191 * t);
        return (a << 24) | (red << 16) | (grn << 8) | blu;
    }
