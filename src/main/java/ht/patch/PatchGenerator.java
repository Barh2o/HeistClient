...
public class PatchGenerator {
    ...
    public static void captureGamepackDiff(String className, byte[] modified) {
        ...
        if (!BytecodePatcher.isIdentical(original, modified)) {
            ...
            // " (diff: " + diff.length + " bytes, ratio: " + String.format("%.1f%%", ratio
            // * 100) + ")");
        }
    }
    ...
    public static void writePatchesZip(String resourcesPath) throws Exception {
        ...
        // Create resources/ht/heist directory — must match HeistClient.class package
        // PatchApplier uses HeistClient.class.getResourceAsStream("patches.zip") which
        // resolves to ht/heist/patches.zip
        Path HeistClientDir = Paths.get(resourcesPath, "ht", "heist");
        Files.createDirectories(HeistClientDir);

        // Delete old zip if exists
        Path zipPath = HeistClientDir.resolve("patches.zip");
    ...
    private static String generateMetadata() {
        ...
        sb.append("# HeistClient Binary Patches\n");
