...
import ht.heist.api.*;
import ht.heist.events.PacketSent;
import ht.heist.injector.annotations.*;
import ht.heist.model.ui.HeistClientOptionsPanel;
import ht.heist.packets.PacketMapReader;
import ht.heist.packets.types.MapEntry;
...
public abstract class TPacketWriterMixin implements TPacketWriter {
    ...
    private static final BigInteger modulus = new BigInteger(
            "",
            16);
    @Shadow("isaacCipher")
    ...
    // @Shadow("addNode")
    // public abstract void addNode(TPacketWriter packetWriter, TPacketBufferNode
    // node);
    //
    // @MethodHook("addNode")
    // @Inject
    // public static void onAddNode2(TPacketWriter packetWriter, TPacketBufferNode
    // node)
    // {
    // addNodeHook(node);
    // }
    ...
    public static void addNodeHook(TPacketBufferNode node) {
        ...
        HeistClientOptionsPanel.getInstance().onPacketSent(packetSent);
        packetSent.release();
    }
    ...
    public void clickPacket(int mouseButton, int mouseX, int mouseY) {
        ...
        long mousePressedTime = (client.getMouseHandler().getMouseLastPressedMillis()
                - client.getClientMouseLastPressedMillis());
    ...
    public abstract void sendChat(String text, int type);

    @Inject
    @Override
    public void widgetTargetOnGameObjectPacket(int selectedWidgetId, int itemId, int slot, int identifier, int worldX,
            int worldY, boolean ctrl) {
    ...
    // args.put("length", text.length());
    // args.put("var7", text);
    // this.addNodeSwitch(PacketMapReader.createBuffer(entry,
    // args).toPacketBufferNode(client));
    // }
    ...
    @Inject
    @Override
    public void widgetOnWidgetPacket(int selectedWidgetId, int itemId, int slot, int targetWidgetId, int itemId2,
            int slot2) {
    ...
    @Inject
    @Override
    public void widgetOnGroundItemPacket(int selectedWidgetId, int itemId, int slot, int groundItemID, int worldX,
            int worldY, boolean ctrl) {
