...
import ht.heist.injector.util.LogCallRemover;
...
public class PluginManagerMixin {
        ...
        @Insert(method = "loadSideLoadPlugins", at = @At(value = AtTarget.GETFIELD, owner = "net/runelite/client/plugins/PluginManager", target = "developerMode"), raw = true)
        public static void loadSideLoadPlugins(MethodNode method, AbstractInsnNode insertionPoint) {
                if (Static.getCliArgs().isNoPlugins() || Static.getCliArgs().isSafeLaunch()) {
                ...
                for (AbstractInsnNode node : method.instructions) {
                        if (node instanceof TypeInsnNode) {
                                ...
                                typeInsn.desc = "ht/heist/services/hotswapper/PluginClassLoader";
                                continue;
                        }
                        ...
                        if (node instanceof MethodInsnNode) {
                                ...
                                if (methodInsn.owner.equals("net/runelite/client/plugins/PluginClassLoader")) {
                                        methodInsn.owner = "ht/heist/services/hotswapper/PluginClassLoader";
                                }
                        }
                }
        }

        @Insert(method = "loadPlugins", at = @At(value = AtTarget.GETFIELD, owner = "net/runelite/client/plugins/PluginManager", target = "developerMode"), raw = true)
        public static void loadPlugins(MethodNode method, AbstractInsnNode insertionPoint) {
                ...
                // Replace "getSuperclass() == Plugin.class" with
                // "Plugin.class.isAssignableFrom(getSuperclass())"
                ...
                if (target != null) {
                        ...
                        instructions.insert(target,
                                        new LdcInsnNode(Type.getType("Lnet/runelite/client/plugins/Plugin;")));
                        ...
                        instructions.insert(target.getNext().getNext(), new MethodInsnNode(
                                        ...
                                        "(Ljava/lang/Class;)Z",
                                        false));
                        instructions.insert(target.getNext().getNext().getNext(),
                                        new JumpInsnNode(Opcodes.IFNE, label));
                }
        }

        @Insert(method = "loadCorePlugins", at = @At(value = AtTarget.STORE, local = 2), ordinal = -1, raw = true)
        public static void loadCorePlugins(MethodNode method, AbstractInsnNode insertionPoint) {
                InsnList code = BytecodeBuilder.create()
                                ...
                                .newInstance("ht/heist/model/RuneLite")
                                .dup()
                                .getStaticField(
                                                "ht/heist/heistclient/Main",
                                                "CLASSLOADER",
                                                "Lht/heist/classloader/RLClassLoader;")
                                .invokeVirtual(
                                                "ht/heist/classloader/RLClassLoader",
                                                "getMain",
                                                "()Ljava/lang/Class;")
                                .invokeSpecial(
                                                "ht/heist/model/RuneLite",
                                                "<init>",
                                                "(Ljava/lang/Class;)V")
                                .pushString("RL")
                                .invokeStatic(
                ...
                InsnList code2 = BytecodeBuilder.create()
                                ...
                                .invokeStatic("ht/heist/heistclient/Main", "isMinMode", "()Z")
                                .storeLocal(7, ISTORE)

                                // Store the shaded jar check result in local var 8
                                .invokeStatic("ht/heist/Static", "isRunningFromShadedJar", "()Z")
                                .storeLocal(8, ISTORE)
                                ...
                                .whileBlock(
                                                ...
                                                body -> {
                                                        ...
                                                        LabelNode skipDevtoolsCheck = body
                                                                        .createLabel("skipDevtoolsCheck");

                                                        // Devtools filter: skip if running from shaded jar AND plugin
                                                        // is devtools
                                                        body.loadLocal(8, Opcodes.ILOAD)
                                                                        .pushInt(0)
                                                                        .jumpIf(ConditionType.EQUALS,
                                                                                        skipDevtoolsCheck); // If not
                                                                                                            // shaded
                                                                                                            // jar, skip
                                                                                                            // this
                                                                                                            // check
                                                        ...
                                                        body.loadLocal(6, ALOAD)
                                                                        ...
                                                                        .invokeVirtual("java/lang/String", "startsWith",
                                                                                        "(Ljava/lang/String;)Z")
                                                                        .pushInt(0)
                                                                        .jumpIf(ConditionType.NOT_EQUALS, skipAdd); // If
                                                                                                                    // is
                                                                                                                    // devtools,
                                                                                                                    // skip
                                                                                                                    // plugin
                                                        ...
                                                        body.loadLocal(7, Opcodes.ILOAD)
                                                                        ...
                                                                        .jumpIf(ConditionType.EQUALS, checkDisallowed); // if
                                                                                                                        // minMode
                                                                                                                        // ==
                                                                                                                        // 0
                                                                                                                        // (false),
                                                        ...
                                                        for (String prefix : allowed) {
                                                                body.loadLocal(6, ALOAD)
                                                                                .pushString(prefix)
                                                                                .invokeVirtual("java/lang/String",
                                                                                                "startsWith",
                                                                                                "(Ljava/lang/String;)Z")
                                                                                .or();
                                                        }
                                                        body.pushInt(0) // Push 0 for comparison
                                                                        .jumpIf(ConditionType.NOT_EQUALS, addPlugin) // if
                                                                                                                     // result
                                                                                                                     // !=
                                                                                                                     // 0,
                                                                                                                     // add
                ...
                method.maxLocals = Math.max(method.maxLocals, 9); // Ensure we have enough local variable slots (added
                                                                  // local 8 for shaded jar check)
        }
}
