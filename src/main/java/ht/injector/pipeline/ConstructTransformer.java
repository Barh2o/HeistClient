...
import ht.heist.injector.annotations.Construct;
import ht.heist.injector.annotations.Mixin;
import ht.heist.injector.util.AnnotationUtil;
import ht.heist.injector.util.TransformerUtil;
...
/**
 * Transformer that processes @Construct annotations to generate factory methods
 * for gamepack class construction.
 * Converts annotated method signatures into appropriate constructor calls with
 * parameter mapping and type casting.
 */
public class ConstructTransformer {
    /**
     * Patches a mixin method annotated with @Construct to create an instance of the
     * target class.
     * Generates a factory method that creates and returns a new instance using the
     ...
     * @throws RuntimeException if target classes or matching constructors cannot be
     *                          found
     */
    public static void patch(ClassNode mixin, MethodNode method) {
        ...
        // Fallback: obfuscated constructors often have a trailing garbage primitive
        // (byte/int).
        // Try to find a constructor with one extra trailing primitive param.
        if (targetConstructor == null) {
            targetConstructor = findMatchingConstructorWithGarbage(constructTarget, paramTypes);
            if (targetConstructor != null) {
                System.out.println(
                        "[ConstructTransformer] INFO: Using garbage-param constructor for " + constructTarget.name);
            }
        }

        if (targetConstructor == null) {
            System.out.println("[ConstructTransformer] WARN: No matching constructor found in " + constructTarget.name +
                    " for parameters " + Arrays.toString(paramTypes) + " — skipping.");
            return;
        }
    ...
    /**
     * Finds a constructor in the target class that matches the provided parameter
     * types.
     *
    ...
    /**
     * Finds a constructor accepting the supplied params plus one trailing garbage
     * primitive.
     * Obfuscators commonly add a trailing byte/int/short parameter to obfuscate
     * constructors.
     */
    private static MethodNode findMatchingConstructorWithGarbage(ClassNode classNode, Type[] paramTypes) {
        for (MethodNode method : classNode.methods) {
            if (!"<init>".equals(method.name))
                continue;
            Type[] cp = Type.getArgumentTypes(method.desc);
            if (cp.length != paramTypes.length + 1)
                continue;
            // Last param must be a primitive (byte/short/int/long)
            int lastSort = cp[cp.length - 1].getSort();
            if (lastSort != Type.BYTE && lastSort != Type.SHORT && lastSort != Type.INT && lastSort != Type.LONG)
                continue;
            // First N params must match
            Type[] prefix = Arrays.copyOf(cp, paramTypes.length);
            if (isCompatibleSignature(paramTypes, prefix))
                return method;
        }
        return null;
    }
    ...
    /**
     * Checks if the provided parameter types are compatible with the expected
     * parameter types.
     *
