...
import ht.heist.injector.types.FieldHookDef;
...
public class FieldHookTransformer {
    ...
    public static void patch(ClassNode mixin, MethodNode method) {
        ...
        if (jField == null) {
            System.out.println(
                    "[FieldHookTransformer] WARN: No field mapping for " + gamepackName + "." + name + " — skipping.");
            return;
        }
    ...
    private static List<AbstractInsnNode> getInjectionPoints(MethodNode method, FieldHookDef hook, boolean isStatic) {
        ...
        for (AbstractInsnNode insn = method.instructions.getFirst(); insn != null; insn = insn.getNext()) {
            ...
            if ((isStatic && insn.getOpcode() == Opcodes.PUTSTATIC)
                    || (!isStatic && insn.getOpcode() == Opcodes.PUTFIELD)) {
