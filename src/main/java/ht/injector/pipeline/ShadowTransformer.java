...
public class ShadowTransformer {
    ...
    public static void patch(ClassNode mixin, MethodNode method) {
        ...
        if (gamepack == null) {
            System.out.println("[ShadowTransformer] WARN: No method mapping for " + gamepackName + "." + name
                    + " — skipping shadow method.");
            return;
        }

        MethodNode toShadow = TransformerUtil.getTargetMethod(mixin, name);
        if (toShadow == null) {
            System.out.println("[ShadowTransformer] WARN: Target method " + gamepackName + "." + name
                    + " not found in gamepack — skipping.");
            return;
        }
    ...
    public static void patch(ClassNode mixin, FieldNode field) {
        try {
            ...
            if (jField == null) {
                System.out.println(
                        "[ShadowTransformer] WARN: No field mapping for " + gamepackName + "." + name + " — skipping.");
                return;
            }
            ...
            e.printStackTrace();
            // Don't System.exit() — log and continue so other mixins can still be applied
        }
    }
     ...
     */
    private static void transformProxyField(JField field, MethodNode mn, Type oldType, Type newType, String oldName,
            String oldOwner, String newName, String newOwner) {
        ...
        for (AbstractInsnNode insn = instructions.getFirst(); insn != null; insn = insn.getNext()) {
            ...
            if (!fin.owner.equals(oldOwner) || !fin.name.equals(oldName))
                continue;
    ...
    public static void createGenericBridgeMethod(ClassNode classNode,
        ...
        MethodNode bridgeMethod = new MethodNode(
                ...
                interfaceMethod.exceptions != null ? interfaceMethod.exceptions.toArray(new String[0]) : null);
