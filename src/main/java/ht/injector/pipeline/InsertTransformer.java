...
import ht.heist.injector.util.MethodUtil;
...
import ht.heist.injector.annotations.Shift;
import ht.heist.injector.types.InstructionMatcher;
...
public class InsertTransformer {
    ...
    public static void patch(ClassNode mixin, MethodNode method) {
        try {
            ...
            if (gamepackClass == null) {
                System.out.println("[InsertTransformer] WARN: No method mapping for " + gamepackName + "."
                        + targetMethodName + " — skipping.");
                return;
            }
            ...
            if (matches.isEmpty()) {
                System.err.println("No matching instructions found for pattern in method: " + targetMethod.name + " :: "
                        + targetMethodName + targetMethod.desc);
                return;
            }
            ...
            if (insertAnnotation.raw()) {
                ...
                System.out
                        .println("Successfully invoked raw insert method: " + method.name + " on " + targetMethod.name);
                return;
            }
            ...
            System.out.println("Successfully inserted " + targetMatches.size() + " call(s) to " + method.name + " in "
                    + targetMethod.name);
    ...
    private static void insertMethodCall(MethodNode targetMethod, AbstractInsnNode insertPoint,
        ...
        InsnList callInstructions = MethodUtil.generateContextAwareInvoke(gamepackClass, targetMethod, injectedMethod,
                true);
