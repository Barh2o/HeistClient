...
import ht.heist.util.asm.ClassNodeUtil;
...
import ht.heist.injector.pipeline.*;
import ht.heist.util.JarDumper;
import ht.heist.util.PackageUtil;
...
public class Injector {
    private static final String MIXINS = "ht.heist.mixins";
    ...
    public static void patch() throws Exception {
        ...
        System.out.println(
                "Remaining " + (totalClasses - mixinTargets.size()) + " classes use optimized loading (skip frames)");
        ...
        for (var entry : Main.LIBS.getGamepack().classes.entrySet()) {
            ...
            // Mixin targets need full frame expansion, others can skip frames (saves
            // 200-300MB)
        ...
        // Optimized output phase: Process and clear classes one-by-one to reduce memory
        // pressure
    ...
    /**
     * Scans mixin annotations to identify which gamepack classes will be
     * transformation targets.
     * These classes need full frame expansion. All others can use SKIP_FRAMES for
     * memory savings.
     */
    ...
    private static void applyMixins(HashMap<ClassNode, ClassNode> pairs) throws ClassNotFoundException {
        for (ClassNode mixin : pairs.keySet()) {
            ...
            if (isCheatIdentifier) {
                ...
                if (clazz == null) {
                    System.out.println("Warning: Could not find mapping for mixin target class: " + gamepackName);
                    continue;
                }
                gamepackClass = gamepack.get(clazz.getObfuscatedName());
            }
            if (gamepackClass == null) {
                System.out.println("Warning: Could not find target class for mixin: " + gamepackName);
                continue;
            }
