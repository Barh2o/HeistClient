package ht.heist.services.profiler;

import ht.heist.model.ui.components.VitaFrame;
...
import ht.heist.services.profiler.recording.RecordingTab;
...
public class ProfilerWindow extends VitaFrame {
    ...
    private ht.heist.services.profiler.leak.LeakDetectorTab leakDetectorTab;

    // GC Pause Analysis
    private ht.heist.services.profiler.gc.GCPauseAnalyzer gcPauseAnalyzer;
    private ht.heist.services.profiler.gc.GCTuningAdvisor gcTuningAdvisor;
    private JTextArea gcStatsArea;
    private JTextArea gcRecommendationsArea;

    // Event Timeline
    private ht.heist.services.profiler.timeline.TimelinePanel timelinePanel;
    ...
    public ProfilerWindow() {
        super("HeistClient JVM Profiler");
        setTitle("HeistClient JVM Profiler");
        setSize(900, 700);
    ...
    private void initializeComponents() {
        ...
        leakDetectorTab = new ht.heist.services.profiler.leak.LeakDetectorTab();
        tabbedPane.addTab("Leak Detector", leakDetectorTab);
        timelinePanel = new ht.heist.services.profiler.timeline.TimelinePanel(gcPauseAnalyzer);
    ...
    private JPanel createResourceMonitorPanel() {
        ...
        gcPauseAnalyzer = new ht.heist.services.profiler.gc.GCPauseAnalyzer(500);
        gcTuningAdvisor = new ht.heist.services.profiler.gc.GCTuningAdvisor(gcPauseAnalyzer);
    ...
    private JPanel createStatusBar() {
        ...
        apiToggleBtn.addActionListener(e -> {
            ht.heist.services.profiler.server.ProfilerServer server = ht.heist.services.profiler.server.ProfilerServer.getInstance();
            if (server.isRunning()) {
                server.stop();
    ...
    private void updateGCAnalysis() {
        ...
        try {
            // Update statistics
            ht.heist.services.profiler.gc.GCPauseAnalyzer.GCStatistics stats = gcPauseAnalyzer.getStatistics();
            ...
            java.util.List<ht.heist.services.profiler.gc.GCTuningAdvisor.Recommendation> recommendations =
                gcTuningAdvisor.getRecommendations();

            StringBuilder recSb = new StringBuilder();

            for (ht.heist.services.profiler.gc.GCTuningAdvisor.Recommendation rec : recommendations) {
