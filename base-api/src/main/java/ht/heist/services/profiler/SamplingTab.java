...
import ht.heist.services.profiler.sampling.*;
...
public class SamplingTab extends JPanel {
    ...
    // Flame graph
    private ht.heist.services.profiler.visualization.FlameGraphPanel flameGraphPanel;
    ...
    private void initializeSamplers() {
        ...
        flameGraphPanel = new ht.heist.services.profiler.visualization.FlameGraphPanel();
    }
    ...
    private void buildFlameGraph() {
        List<ht.heist.services.profiler.sampling.StackSample> samples = cpuSampler.getStackSamples().getAll();
        if (samples.isEmpty()) {
            return;
        }

        // Create root node
        ht.heist.services.profiler.visualization.FlameGraphNode root =
            new ht.heist.services.profiler.visualization.FlameGraphNode("(root)", "", "");

        // Build call tree from stack samples
        for (ht.heist.services.profiler.sampling.StackSample sample : samples) {
            ...
            ht.heist.services.profiler.visualization.FlameGraphNode current = root;
            current.addSample();
