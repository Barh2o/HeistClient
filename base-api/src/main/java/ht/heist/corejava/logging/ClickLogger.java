...
public final class ClickLogger {
    ...
    public ClickLogger() {
        this("default", ht.heist.Static.VITA_DIR.toFile(), true);
    }
    ...
    public synchronized void start() throws IOException {
        ...
        jsonl = new BufferedWriter(
                new OutputStreamWriter(new FileOutputStream(jsonlFile, true), StandardCharsets.UTF_8));

        if (alsoCsv) {
            final File csvFile = new File(outDir, "clicks-" + sessionId + "-" + stamp + ".csv");
            csv = new BufferedWriter(
